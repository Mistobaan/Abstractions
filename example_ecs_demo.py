...
def main():
    ...
    player.data["health"] = {"max_hp": 100, "current_hp": 75, "regeneration_rate": 2}

    # Position Component (data only)
    player.data["position"] = {"x": 10.5, "y": 20.3, "z": 0.0}
    ...
    game_world = abs.EntityTree(root_ecs_id=player.ecs_id, lineage_id=player.lineage_id)
    ...
    print(
        f"Player now wields weapon: {player.data['equipment']['main_hand'] == weapon.ecs_id}"
    )
    print()
    ...
    def movement_system(
        entity_id: UUID, dx: float, dy: float, world: abs.EntityTree
    ) -> str:
        """
    ...
    # Query examples using EntityTree methods
    entities_with_health = game_world.find_entities_with_component("health")
    entities_with_items = game_world.find_entities_with_component("item")
    player_compositions = game_world.find_entities_by_relationship(
        player.ecs_id, abs.EdgeType.COMPOSITION
    )

    # Alternative: using root-level functions (with explicit tree)
    entities_with_health_alt = abs.find_entities_with_component("health", game_world)
    player_compositions_alt = abs.find_entities_by_relationship(
        player.ecs_id, abs.EdgeType.COMPOSITION, game_world
    )

    # Verify both approaches give same results
    assert entities_with_health == entities_with_health_alt
    assert player_compositions == player_compositions_alt
    ...
    # Demonstrate default tree functionality
    print("\nDefault Tree Demonstration:")
    abs.set_default_tree(game_world)  # Set our game world as default

    # Now we can query without specifying a tree
    default_health_entities = abs.find_entities_with_component("health")
    default_compositions = abs.find_entities_by_relationship(
        player.ecs_id, abs.EdgeType.COMPOSITION
    )

    print(f"Using default tree - Health entities: {len(default_health_entities)}")
    print(f"Using default tree - Player compositions: {len(default_compositions)}")
    print()
    ...
    player.data["experience"] = {"level": 5, "current_xp": 1250, "xp_to_next": 2000}
    ...
    print("✓ Efficient querying and filtering with EntityTree methods")
    print("✓ Root-level query functions with default tree support")
    print("✓ Separation of data and behavior")
    print("✓ Tree-based entity organization")
    print()
    print("=== QUERY METHODS DEMONSTRATED ===")
    print("-" * 50)
    print("✓ EntityTree.find_entities_with_component()")
    print("✓ EntityTree.find_entities_by_relationship()")
    print("✓ abs.find_entities_with_component() with explicit tree")
