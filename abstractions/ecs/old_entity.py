...
def compare_entity_fields(
    entity1: Any, entity2: Any, exclude_fields: Optional[Set[str]] = None
    ...
    logger.info(
        f"Comparing entities: {type(entity1).__name__}({entity1.ecs_id}) vs {type(entity2).__name__}({entity2.ecs_id})"
    )
    ...
    logger.debug(
        f"Comparing {len(entity1_fields)} fields after excluding implementation fields"
    )
    ...
    if entity1_fields != entity2_fields:
        ...
        logger.info(
            f"Schema difference detected: fields {diff_fields} differ between entities"
        )
    ...
    for field in entity1_fields:
        ...
        if isinstance(value1, Entity) and isinstance(value2, Entity):
            if value1.ecs_id != value2.ecs_id:
                has_diffs = True
                logger.info(
                    f"Field '{field}' contains different entities: {value1.ecs_id} vs {value2.ecs_id}"
                )
                field_diffs[field] = {
                    "type": "modified",
        ...
        # If both are lists, compare items individually
            ...
            if len(value1) != len(value2):
                has_diffs = True
                logger.info(
                    f"Field '{field}' has different list lengths: {len(value1)} vs {len(value2)}"
                )
                field_diffs[field] = {
                    "type": "modified",
                ...
                if all(isinstance(v, Entity) for v in value1) and all(
                    isinstance(v, Entity) for v in value2
                ):
                    ...
                    if ids1 != ids2:
                        has_diffs = True
                        logger.info(
                            f"Field '{field}' contains lists of different entities"
                        )
        ...
        # Special handling for datetime objects to handle timezone differences
            ...
            if v1_normalized != v2_normalized:
                has_diffs = True
                logger.info(
                    f"Field '{field}' has different datetime values (after normalization): {v1_normalized} vs {v2_normalized}"
                )
                field_diffs[field] = {
                    "type": "modified",
                    "old": v2_normalized,
        ...
        # For all other types, compare normally
            ...
            field_diffs[field] = {"type": "modified", "old": value2, "new": value1}

    logger.info(
        f"Comparison result: has_diffs={has_diffs}, found {len(field_diffs)} different fields"
    )
...
class EntityDiff:
    ...
    def add_diff(
        self, field: str, diff_type: str, old_value: Any = None, new_value: Any = None
    ) -> None:
    ...
    def has_changes(self) -> bool:
        ...
        logger.info(
            f"Checking significance of {len(self.field_diffs)} field differences"
        )
        ...
        for field, diff_info in self.field_diffs.items():
            ...
            if field in {
                ...
                logger.debug(
                    f"Field '{field}' is an implementation detail - not significant"
                )
                continue

            # Any other field change is significant
            logger.info(
                f"Field '{field}' has significant changes (type: {diff_info.get('type', 'unknown')})"
            )
            return True

        # No significant changes found
        logger.info(
            "No significant changes found - all differences are implementation details"
        )
        return False
...
class GraphNode(BaseModel):
    ...
    dependencies: Set[Any] = Field(
        default_factory=set
    )  # IDs of entities this entity depends on
    dependents: Set[Any] = Field(
        default_factory=set
    )  # IDs of entities that depend on this entity
...
class EntityDependencyGraph(BaseModel):
    ...
    nodes: Dict[Any, GraphNode] = Field(
        default_factory=dict
    )  # Map of entity ID to its node
    ...
    def build_graph(
        ...
        get_entity_id: Optional[Callable[[Any], Any]] = None,
    ) -> CycleStatus:
        """
        ...
        entity_dependencies: Dict[Any, Set[Any]] = (
            {}
        )  # Map entity ID to set of dependency IDs
        ...
        while entities_to_process:
            ...
            deps = self._find_entity_references(
                entity, is_entity_check_func, entity_id_func
            )
            dep_ids = set()
    ...
    def _find_entity_references(
        ...
        entity_id_func: Callable[[Any], Any],
    ) -> List[Tuple[Any, str]]:
        """
    ...
    def get_topological_sort(self) -> List[Any]:
        ...
        sorted_nodes = sorted(
            self.nodes.keys(), key=lambda node_id: depths.get(node_id, 0)
        )
...
class Entity(BaseModel):
    ...
    from_storage: bool = Field(
        default=False, description="Whether the entity was loaded from storage"
    )
    force_parent_fork: bool = Field(
        default=False, description="Internal flag to force parent forking"
    )
    # Always set to True by default to ensure all entities are registered properly
    root_entity: bool = Field(
        default=True, description="Whether the entity is the root of an entity tree"
    )
    ...
    def initialize_deps_graph(self) -> None:
        ...
        logger.info(
            f"Initializing dependency graph for {type(self).__name__}({self.ecs_id})"
        )
    ...
    def fork(self: T_Self) -> T_Self:
        ...
        for entity in entities_to_fork:
            ...
            if hasattr(entity, "deps_graph") and entity.deps_graph is not None:
                ...
                if node:
                    deps = [
                        entity.deps_graph.find_entity_by_id(dep_id)
                        for dep_id in node.dependencies
                    ]
        ...
        logger.info(
            f"Sorted {len(sorted_entities)} entities for forking in dependency order"
        )
        ...
        for entity in sorted_entities:
            ...
            logger.debug(
                f"Forked entity {type(entity).__name__}: {old_id} -> {entity.ecs_id}"
            )
            ...
            for parent in sorted_entities:
                ...
                for field_name, field_info in parent.model_fields.items():
                    ...
                    if isinstance(value, Entity) and value.ecs_id == old_id:
                        ...
                        logger.debug(
                            f"Updated reference in {parent.ecs_id}.{field_name}"
                        )
                    ...
                    # List/tuple of entities
                        ...
                        for i, item in enumerate(value):
                            if isinstance(item, Entity) and item.ecs_id == old_id:
                                value[i] = entity
                                logger.debug(
                                    f"Updated reference in {parent.ecs_id}.{field_name}[{i}]"
                                )
                    ...
                    # Dict containing entities
                        ...
                        for k, v in value.items():
                            if isinstance(v, Entity) and v.ecs_id == old_id:
                                value[k] = entity
                                logger.debug(
                                    f"Updated reference in {parent.ecs_id}.{field_name}[{k}]"
                                )
                        setattr(parent, field_name, value)
            ...
            if b_entity is not None and hasattr(b_entity, "ref_to_c"):
                ...
                if c_entity is not None and hasattr(c_entity, "ref_to_a"):
                    ...
                    if ref_to_a is not None and ref_to_a.ecs_id != a_entity.ecs_id:
                        ...
                        logger.debug(
                            f"Fixed circular reference C->A to point to new A ({a_entity.ecs_id})"
                        )
    ...
    def has_modifications(
        self, other: "Entity"
    ) -> Tuple[bool, Dict["Entity", EntityDiff]]:
        """
        ...
        logger.info(
            f"Checking modifications: {type(self).__name__}({self.ecs_id}) vs {type(other).__name__}({other.ecs_id}) ]"
        )
        ...
        if other is not None and (
            not hasattr(other, "deps_graph") or other.deps_graph is None
        ):
        ...
        sorted_entities = (
            self.deps_graph.get_topological_sort() if self.deps_graph else []
        )
        ...
        for entity in sorted_entities:
            ...
            if entity.ecs_id not in other_entities:
                logger.debug(
                    f"Entity {entity.ecs_id} not found in other tree, skipping"
                )
                continue
            ...
            if has_diffs:
                logger.info(
                    f"Direct field differences found in {type(entity).__name__}({entity.ecs_id})"
                )
                ...
                if significant_changes:
                    logger.info(
                        f"Entity {type(entity).__name__}({entity.ecs_id}) has significant changes requiring fork"
                    )
                    ...
                    if entity in modified_entities:
                        ...
                        modified_entities[entity] = EntityDiff.from_diff_dict(
                            field_diffs
                        )
                    ...
                    if hasattr(entity, "deps_graph") and entity.deps_graph is not None:
                        ...
                        for parent_id in parent_ids:
                            ...
                            if parent and parent not in modified_entities:
                                logger.info(
                                    f"Marking parent {type(parent).__name__}({parent.ecs_id}) for forking due to child changes"
                                )
                                modified_entities[parent] = EntityDiff()

        needs_fork = bool(modified_entities)
        logger.info(
            f"Modification check result: needs_fork={needs_fork}, modified_entities={len(modified_entities)}"
        )
                ...
                for item in value:
                    if isinstance(item, Entity):
                        ...
                        if item.ecs_id not in visited:
                            ...
                            branch_visited = set(
                                visited
                            )  # Use set constructor instead of .copy()
...
class EntityRegistry(BaseRegistry):
    ...
    @classmethod
    def get(
        cls, entity_id: UUID, expected_type: Optional[Type[Entity]] = None
    ) -> Optional[Entity]:
        """
        ...
        if expected_type:
            ...
            if actual_type.__name__ != expected_type.__name__:
                ...
                if not isinstance(ent, expected_type):
                    cls._logger.error(
                        f"Type mismatch: got {actual_type.__name__}, expected {expected_type.__name__}"
                    )
                    return None
    ...
    def list_by_type(cls, entity_type: Type[Entity]) -> List[Entity]:
        ...
        return [
            deepcopy(e) for e in cls._registry.values() if isinstance(e, entity_type)
        ]

    @classmethod
    def get_many(
        cls, entity_ids: List[UUID], expected_type: Optional[Type[Entity]] = None
    ) -> List[Entity]:
        return [
            e for eid in entity_ids if (e := cls.get(eid, expected_type)) is not None
        ]
    ...
    def get_lineage_tree_sorted(cls, lineage_id: UUID) -> Dict[str, Any]:
        ...
        # Find root node
        root_candidates = [
            node_id for node_id, data in tree.items() if not data["parent_id"]
        ]
    ...
    def scan(obj: Any, path: str = "") -> None:
        if isinstance(obj, Entity):
            ...
            if obj.ecs_id not in scanned_ids:
                ...
                entities[id(obj)] = obj
                logger.debug(
                    f"Found entity {type(obj).__name__}({obj.ecs_id}) at path {path}"
                )
        elif isinstance(obj, (list, tuple, set)):
...
def _check_and_process_entities(
    entities: Dict[int, Entity], fork_if_modified: bool = True
) -> None:
    """
    ...
    logger.info(
        f"Processing {len(entities)} entities, fork_if_modified={fork_if_modified}"
    )
    ...
    for entity_id, entity in entities.items():
        ...
        for sub in entity.get_sub_entities():
            ...
            if nested_id in entities:
                ...
                logger.debug(
                    f"Dependency: {type(entity).__name__}({entity.ecs_id}) depends on {type(sub).__name__}({sub.ecs_id})"
                )
    ...
    def process_entity(entity_id: int) -> None:
        ...
        if cold:
            ...
            if needs_fork and fork_if_modified:
                logger.info(
                    f"Entity {type(entity).__name__}({entity.ecs_id}) has modifications, forking"
                )
                forked = entity.fork()
                logger.debug(f"Forked to new entity {forked.ecs_id}")
            else:
                logger.debug(
                    f"Entity {type(entity).__name__}({entity.ecs_id}) has no modifications or fork_if_modified=False"
                )
        else:
...
def entity_tracer(func: Callable[..., Any]) -> Callable[..., Any]:
    ...
    try:
        ...
        is_async = hasattr(func, "__await__") or (
    ...
    async def async_wrapper(*args: Any, **kwargs: Any) -> Any:
        ...
        for entity_id, entity in input_entities.items():
            logger.debug(
                f"Checking input entity {type(entity).__name__}({entity.ecs_id}) before function call"
            )
            ...
            if cold_snapshot:
                ...
                if needs_fork:
                    logger.info(
                        f"Input entity {type(entity).__name__}({entity.ecs_id}) needs fork - forking before call (in-memory mode)"
                    )
                    entity.fork()
                    fork_count += 1
            else:
        ...
        logger.info(
            f"Forked {fork_count} input entities before calling {func.__name__}"
        )
        ...
        logger.info(
            f"Found {len(new_entities_from_lineage)} new entities in output by lineage_id"
        )
        ...
        for entity in new_entities_from_lineage.values():
            if not EntityRegistry.has_entity(entity.ecs_id):
                logger.warning(
                    f"Registering new output entity: {type(entity).__name__}({entity.ecs_id}) this should not happen, the entity should already be registered at creation"
                )
                EntityRegistry.register(entity)
        ...
        for entity_id, entity in input_entities.items():
            logger.debug(
                f"Checking input entity {type(entity).__name__}({entity.ecs_id}) after function call"
            )
            cold_snapshot = EntityRegistry.get_cold_snapshot(entity.ecs_id)
            if cold_snapshot:
                ...
                if needs_fork:
                    logger.info(
                        f"Input entity {type(entity).__name__}({entity.ecs_id}) needs fork - forking after call (in-memory mode)"
                    )
                    entity.fork()
                    after_fork_count += 1

        logger.info(
            f"Forked {after_fork_count} input entities after calling {func.__name__}"
        )
        ...
        if isinstance(result, Entity):
            ...
            if id(result) in input_entities:
                logger.info(
                    f"Result is an input entity that was modified, returning most recent version"
                )
    ...
    def sync_wrapper(*args: Any, **kwargs: Any) -> Any:
        ...
        for entity in new_entities.values():
            if not EntityRegistry.has_entity(entity.ecs_id):
                logger.info(
                    f"Registering new output entity: {type(entity).__name__}({entity.ecs_id})"
                )
                EntityRegistry.register(entity)
