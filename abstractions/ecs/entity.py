...
class EntityTree(BaseModel):
    ...
    outgoing_edges: Dict[UUID, List[UUID]] = Field(
        default_factory=lambda: defaultdict(list)
    )

    # Incoming edges by target - maps entity.ecs_id to list of source IDs
    incoming_edges: Dict[UUID, List[UUID]] = Field(
        default_factory=lambda: defaultdict(list)
    )
    ...
    def update_live_ids(self) -> None:
        ...
        for node in self.nodes.values():
            if node.live_id != new_root_live_id:
                ...
                self.live_id_to_ecs_id[node.live_id] = (
                    node.ecs_id
                )  # Map new ID to ecs_id
                # Only remove the old ID if it's in the map
...
def get_pydantic_field_type_entities(
    entity: "Entity", field_name: str, detect_non_entities: bool = False
) -> Union[Optional[Type], bool]:
    """
    ...
    if is_entity_container:
        return (
            None if detect_non_entities else None
        )  # We return type above if not detect_non_entities
    ...
    if annotation and not detect_non_entities:
        ...
        try:
            ...
            if annotation == Entity or (
                isinstance(annotation, type) and issubclass(annotation, Entity)
            ):
        ...
        if origin is Union:
            ...
            if inner_type:
                try:
                    if inner_type == Entity or (
                        isinstance(inner_type, type) and issubclass(inner_type, Entity)
                    ):
        ...
        if origin in (list, set, tuple, dict):
            ...
            if origin is dict and len(args) >= 2:
                ...
                value_type = args[1]
                try:
                    if value_type == Entity or (
                        isinstance(value_type, type) and issubclass(value_type, Entity)
                    ):
                ...
                item_type = args[0]
                try:
                    if item_type == Entity or (
                        isinstance(item_type, type) and issubclass(item_type, Entity)
                    ):
    ...
    if not detect_non_entities:
        try:
            ...
            if (
                field_type
                and not origin
                and (field_type == Entity or issubclass(field_type, Entity))
            ):
            ...
            if origin in (list, set, tuple, dict):
                ...
                if origin is dict and len(args) >= 2:
                    ...
                    value_type = args[1]
                    if value_type == Entity or (
                        inspect.isclass(value_type) and issubclass(value_type, Entity)
                    ):
                        return None if detect_non_entities else value_type
                elif origin in (list, set, tuple) and args:
                    # For other containers, check first argument
                    item_type = args[0]
                    if item_type == Entity or (
                        inspect.isclass(item_type) and issubclass(item_type, Entity)
                    ):
                        return None if detect_non_entities else item_type
        except Exception:
...
def build_entity_tree(root_entity: "Entity") -> EntityTree:
    ...
    while to_process:
        ...
        if entity.ecs_id in processed and parent_id is not None:
            raise ValueError(
                f"Circular entity reference detected: {entity.ecs_id}. Circular entities are not supported Entities must form hierarchical trees."
            )
        ...
        if parent_id is not None:
            ...
            if edge_key in tree.edges:
                ...
                if parent_id in ancestry_paths:
                    ...
                    if entity.ecs_id not in ancestry_paths or len(entity_path) < len(
                        ancestry_paths[entity.ecs_id]
                    ):
    ...
    for field_name in entity.model_fields:
        ...
        if (
            get_pydantic_field_type_entities(
                entity, field_name, detect_non_entities=True
            )
            is True
        ):
...
def find_modified_entities(
    new_tree: EntityTree, old_tree: EntityTree, greedy: bool = True, debug: bool = False
...
def generate_mermaid_diagram(tree: EntityTree, include_attributes: bool = False) -> str:
    ...
    for edge_key, edge in tree.edges.items():
        ...
        edge_line = (
            f"  {source_id} {edge_style}|{edge_label}| {target_id}:::hierarchicalEdge"
        )
...
class EntityRegistry:
    ...
    def register_entity(cls, entity: "Entity") -> None:
        ...
        if entity.root_ecs_id is None:
            raise ValueError(
                "can only register root entities with a root_ecs_id for now"
            )
    ...
    def version_entity(cls, entity: "Entity", force_versioning: bool = False) -> bool:
        ...
        if not entity.root_ecs_id:
            raise ValueError(
                "entity has no root_ecs_id for versioning we only support versioning of root entities for now"
            )
        ...
        if old_tree is None:
            ...
            if force_versioning:
                ...
                modified_entities = list(
                    find_modified_entities(new_tree=new_tree, old_tree=old_tree)
                )

            typed_entities = [
                entity for entity in modified_entities if isinstance(entity, UUID)
            ]

            if len(typed_entities) > 0:
                if new_tree.root_ecs_id not in typed_entities:
                    raise ValueError(
                        "if any entity is modified the root entity must be modified something went wrong"
                    )
                ...
                if root_entity is None:
                    raise ValueError(
                        "root entity not found in new tree, something went very wrong"
                    )
                root_entity.update_ecs_ids()
                new_root_ecs_id = root_entity.ecs_id
                root_entity_live_id = root_entity.live_id
                assert (
                    new_root_ecs_id is not None
                    and new_root_ecs_id != current_root_ecs_id
                )
                ...
                for modified_entity_id in typed_entities:
                    ...
                    if modified_entity is not None:
                        ...
                        modified_entity.update_ecs_ids(
                            new_root_ecs_id, root_entity_live_id
                        )
                    else:
                        # later here we will handle the case where the entity has been moved to a different tree or prompoted to it's own tree
                        print(
                            f"modified entity {modified_entity_id} not found in new tree, something went wrong"
                        )
...
class Entity(BaseModel):
    ...
    forked_at: Optional[datetime] = Field(
        default=None, description="Timestamp of the last fork"
    )
    previous_ecs_id: Optional[UUID] = Field(
        default=None, description="Previous ecs_id before forking"
    )
    lineage_id: UUID = Field(default_factory=uuid4)
    old_ids: List[UUID] = Field(default_factory=list)
    old_ecs_id: Optional[UUID] = Field(
        default=None, description="Last ecs_id before forking"
    )  # Added this field
    root_ecs_id: Optional[UUID] = Field(
        default=None, description="The ecs_id of the root entity of this entity's tree"
    )
    root_live_id: Optional[UUID] = Field(
        default=None, description="The live_id of the root entity of this entity's tree"
    )
    from_storage: bool = Field(
        default=False,
        description="Whether the entity was loaded from storage, used to prevent re-registration",
    )
    untyped_data: str = Field(
        default="",
        description="Default data container for untyped data, string diff will be used to detect changes",
    )
    attribute_source: Dict[
        str,
        Union[
            Optional[UUID], List[Optional[UUID]], List[None], Dict[str, Optional[UUID]]
        ],
    ] = Field(
        default_factory=dict, description="Tracks the source entity for each attribute"
    )
    ...
    def validate_attribute_source(self) -> Self:
        ...
        if invalid_keys:
            raise ValueError(
                f"Invalid keys in attribute_source: {invalid_keys}. Valid fields are: {valid_fields}"
            )
        ...
        for field_name in valid_fields:
            ...
            if isinstance(field_value, list):
                ...
                typed_dict: Dict[str, Optional[UUID]] = {
                    str(k): None for k in field_value.keys()
                }
        ...
        for field_name, source_value in self.attribute_source.items():
            ...
            if isinstance(field_value, list):
                ...
                if not isinstance(source_value, list) or len(source_value) != len(
                    field_value
                ):
                    none_value: Optional[UUID] = None
                    self.attribute_source[field_name] = [none_value] * len(field_value)
                ...
                if not isinstance(source_value, dict):
                    self.attribute_source[field_name] = {
                        str(k): None for k in field_value.keys()
                    }
                else:
                    ...
                    if isinstance(source_dict, dict):  # Type guard for type checker
                        for key in field_value.keys():
                            str_key = str(
                                key
                            )  # Convert key to string to ensure dict compatibility
    ...
    def update_ecs_ids(
        self,
        new_root_ecs_id: Optional[UUID] = None,
        root_entity_live_id: Optional[UUID] = None,
    ) -> None:
        """
        Assign new ecs_id
        ...
        if root_entity_live_id and not new_root_ecs_id:
            raise ValueError(
                "if root_entity_live_id is provided new_root_ecs_id must be provided"
            )
    ...
    def borrow_attribute_from(
        self, new_entity: "Entity", target_field: str, self_field: str
    ) -> None:
        """
        just a stub for now
    ...
    def attach(self, new_root_entity: "Entity") -> None:
        ...
        if (
            ...
            and self.root_live_id == live_root_entity.live_id
            and self.lineage_id == live_root_entity.lineage_id
        ):
...
class EntityinEntity(Entity):
    ...
    sub_entity: Entity = Field(
        description="The sub entity of the entity", default_factory=Entity
    )
...
class EntityinList(Entity):
    ...
    entities: List[Entity] = Field(
        description="The list of entities of the entity", default_factory=list
    )
...
class EntityinDict(Entity):
    ...
    entities: Dict[str, Entity] = Field(
        description="The dictionary of entities of the entity", default_factory=dict
    )
...
class EntityinTuple(Entity):
    ...
    entities: Tuple[Entity, ...] = Field(
        description="The tuple of entities of the entity"
    )
...
class BaseModelWithEntity(BaseModel):
    ...
    entity: Entity = Field(
        description="The entity of the model", default_factory=Entity
    )
...
class EntityinBaseModel(Entity):
    ...
    base_model: BaseModelWithEntity = Field(
        description="The base model of the entity", default_factory=BaseModelWithEntity
    )
...
class EntityInEntityInEntity(Entity):
    ...
    entity_of_entity: EntityinEntity = Field(
        description="The entity of the entity", default_factory=EntityinEntity
    )
...
class HierarchicalEntity(Entity):
    ...
    entity_of_entity_1: EntityinEntity = Field(
        description="The entity of the entity", default_factory=EntityinEntity
    )
    entity_of_entity_2: EntityinEntity = Field(
        description="The entity of the entity", default_factory=EntityinEntity
    )
    flat_entity: Entity = Field(description="The flat entity", default_factory=Entity)
    entity_of_entity_of_entity: EntityInEntityInEntity = Field(
        description="The entity of the entity of the entity",
        default_factory=EntityInEntityInEntity,
    )
    primitive_data: EntityWithPrimitives = Field(
        description="Entity with primitive data", default_factory=EntityWithPrimitives
    )
