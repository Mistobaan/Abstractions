...
class TestBuildEntityTree(unittest.TestCase):
    ...
    def test_entity_in_entity_tree(self):
        ...
        )  # The edge type should be DIRECT
        ...
        self.assertEqual(tree.ancestry_paths[root.ecs_id], [root.ecs_id])
        self.assertEqual(
            tree.ancestry_paths[sub_entity.ecs_id], [root.ecs_id, sub_entity.ecs_id]
        )
        ...
        self.assertEqual(
            tree.get_hierarchical_children(root.ecs_id), [sub_entity.ecs_id]
        )
    ...
    def test_entity_with_list_tree(self):
        ...
        self.assertEqual(tree.ancestry_paths[root.ecs_id], [root.ecs_id])
        self.assertEqual(
            tree.ancestry_paths[entity1.ecs_id], [root.ecs_id, entity1.ecs_id]
        )
        ...
        self.assertEqual(
            len(children), len(root.entities)
        )  # All children may be marked as hierarchical
    ...
    def test_entity_with_dict_tree(self):
        ...
        self.assertEqual(
            len(hierarchical_keys), len(root.entities)
        )  # All should be hierarchical
    ...
    def test_nested_entity_tree(self):
        ...
        for node_id in tree.nodes:
            ...
            self.assertEqual(
                path[-1], node_id
            )  # Last node in path should be the entity
