...
class TestBasicEntityMovement(unittest.TestCase):
    ...
    def test_basic_movement_workflow(self):
        ...
        print(
            f"Child root_ecs_id == parent2.ecs_id? {child.root_ecs_id == parent2.ecs_id}"
        )
        print(
            f"Child root_live_id == parent2.live_id? {child.root_live_id == parent2.live_id}"
        )

        # Verify the IDs have been updated
        print(
            f"Child ecs_id changed from detached state? {child.ecs_id != child_id_after_detach}"
        )
